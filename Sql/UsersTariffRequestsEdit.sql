-- Создает/Редактирует заявку на оформление Подписки
if exists(select 1 from sysobjects where name = N'UsersTariffRequestsEdit' and xtype='P') drop proc UsersTariffRequestsEdit
go
create proc UsersTariffRequestsEdit (
	@Operation		int,
	@Id				int,
	@UserId			int,
	@TariffId		tinyint,
...
begin	
	...
	begin try 	
		...
		if @Operation = 5 begin		-- Удаляем заявку
		
			delete UsersTariffRequests
			where Id = @Id
